...
//TODO #7
    fetch('/api/ordenes')
        ...
        .then(data => {
            const cuerpoTabla = document.getElementById('ordenesTabla').getElementsByTagName('tbody')[0];
            ...
            data.forEach(orden => {
                ...
                const idOrden = row.insertCell(0);
                const usuario = row.insertCell(1);
                const nomacc = row.insertCell(2);
                const numAcciones = row.insertCell(3);
                const precio = row.insertCell(4);
                const ejecutado = row.insertCell(5);

                idOrden.textContent = orden.id;
                usuario.textContent = orden.usuario;
                nomacc.textContent = orden.nombreAccion;
                numAcciones.textContent = orden.numAcciones;
                precio.textContent = orden.precio;
                ejecutado.textContent  =orden.ejecutado;
